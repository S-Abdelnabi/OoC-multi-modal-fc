...
resnet = resnet_places.PlacesCNN(args.resnet_arch)
resnet.eval()
resnet.cuda()
...
for i in range(start_idx, len(dataset)):
    ...
    if int(key)%2==0 or (int(key)%2!=0 and int(key)-1 not in keys_as_int) :
        ...
        with torch.no_grad():
            resnet_features = resnet(imgs, features='pool')
        save_features(resnet_features,os.path.join(direct_path_item,'places_resnet_features'))   
                
    inv_path_item = os.path.join(args.queries_dataset_root,context_data_items_dict[key]['inv_path'])
        
    qImg = sample['qImg'].cuda() 
    with torch.no_grad():
        qImg_resnet_features = resnet(torch.unsqueeze(qImg,dim=0), features='pool')
    save_features(qImg_resnet_features,os.path.join(inv_path_item,'qImg_places_resnet_features'))   
